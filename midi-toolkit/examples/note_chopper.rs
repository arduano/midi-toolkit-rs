...
fn main() {
    ...
    for (i, track) in file.iter_all_tracks().enumerate() {
        ...
        let cached = track.collect_vec_result().unwrap();

        let non_note_events = cached.iter().cloned().into_ok().filter_non_note_events();
        ...
        let notes = cached
            ...
            .events_to_notes()
            .unwrap_items();
        let chopped = notes.map(|n| chop_note(n, chop_size).into_ok());
        let flattened = chopped.merge_all().notes_to_events();
